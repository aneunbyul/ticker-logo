...
// src/data/kospi-tickers.ts 파일에서 ticker 목록을 가져옴
import { kospiTickers as tickers } from '../src/data/kospi-tickers';
...
(async () => {
  ...
  if (imagesToDownload.length === 0) {
    ...
    const content = `export const kospiTickers: string[] = [];

export default kospiTickers;
`;
    fs.writeFileSync('src/data/kospi-tickers.ts', content);
